...
var normalizer = {
    cleanRow: function(row){
        ...
        if(row && row.value){
            ...
            output.id = row.id;
            output.key = row.key;
            output.value.submitted_by = row.value.submitted_by ? superTrim(row.value.submitted_by) : 'a'; 
            output.value.death = row.value.death ? clean.section.death(row.value.death) : clean.defaults.death;
            output.value.location = row.value.location ? clean.section.location(row.value.location) : clean.defaults.location;
            output.value.subject = row.value.subject ? clean.section.subject(row.value.subject) : clean.defaults.subject; 
